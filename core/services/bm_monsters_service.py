...
def copy_image_to_export(images_folder, image_name, image_type):
    ...
    _log_capture_debug(f"[DEBUG] export image path: {image_path}")
...
def generate_monster_template(captured_images):
    ...
    try:
        ...
        _log_capture_debug(f"[INFO] Calculated scalar threshold: {scalar_threshold}")

        return template, scalar_threshold

    except Exception as e:
        _log_capture_debug(f"[ERROR] Failed to generate template: {e}")
        return None, None
...
def find_optimal_threshold(image):
    ...
    try:
        ...
        _log_capture_debug(f"[ERROR] Failed to find optimal threshold: {e}")
...
def start_simulate_monster_click(thread):
    try:
        ...
        _log_capture_debug(f"[ERROR] Failed to simulate the click: {e}")
...
def capture_template_ss(thread):
    try:
        with open("capture_debug.log", "a", encoding='utf-8', errors='replace') as f:
            f.write(f"[DEBUG] capture_template_ss() called in thread\n")
        ...
        if captured_image is None:
            with open("capture_debug.log", "a", encoding='utf-8', errors='replace') as f:
                f.write(f"[ERROR] take_screenshot returned None\n")
            return
        
        with open("capture_debug.log", "a", encoding='utf-8', errors='replace') as f:
            f.write(f"[DEBUG] Screenshot captured: {captured_image.shape}\n")
        
        # Emit the signal to update the Selection Tool Widget
        qimage = convert_cv_to_qimage(captured_image)
        with open("capture_debug.log", "a", encoding='utf-8', errors='replace') as f:
            f.write(f"[DEBUG] Converted to QImage, emitting frame_ready signal\n")
        thread.ref.frame_ready.emit(qimage)
        with open("capture_debug.log", "a", encoding='utf-8', errors='replace') as f:
            f.write(f"[DEBUG] frame_ready signal emitted successfully\n")
