...
def extract_monster_name_from_image(src_img):
    ...
    try:
        monster_info_text = pytesseract.image_to_string(processed, config=custom_config)
    except Exception:
        return ""

    return monster_info_text.replace(" ", "").replace("\n", "").lower()
...
def lookup_boss_by_name(extracted_monster_name):
    ...
    session = get_session()
    try:
        normalized_extracted = normalize_boss_text(extracted_monster_name or "")
        ...
        matching_ids = [
            ...
            if normalize_boss_text(monster.name) == normalized_extracted
        ]

        # Fuzzy fallback for name variants and OCR drift (e.g. mythicalnian/epicnian)
        if not matching_ids:
            extracted_tokens = [token for token in re.split(r"[^a-z0-9]+", normalized_extracted) if token]
            primary_token = extracted_tokens[-1] if extracted_tokens else normalized_extracted

            for monster in monster_levels:
                normalized_name = normalize_boss_text(monster.name)
                ratio = SequenceMatcher(None, normalized_name, normalized_extracted).ratio()

                token_hit = primary_token and (
                    primary_token in normalized_name or normalized_name in normalized_extracted
                )

                if ratio >= 0.72 or token_hit:
                    matching_ids.append(monster.id)

        if matching_ids:
            bosses = (
                session.query(
                    MonsterLevel,
                    BossMonster.monster_logic_id.label("logic_id")
                )
